...
pub fn build_ffmpeg_linux(enable_nvenc: bool, deps_path: &Path) {
    ...
    if enable_nvenc {
        // NVENC encoders only need the ffnvcodec dynlink headers: CUDA kernels (cuda-nvcc,
        // libnpp) are not used by ALVR and are incompatible with recent CUDA toolkits
        // (compute_52 support was removed in CUDA 13). This matches the upstream master build.
        // The headers are also used by the direct (zero-copy) NVENC pipeline in server_openvr.
        #[cfg(target_os = "linux")]
        {
